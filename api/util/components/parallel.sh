...
run_one() (
	...
	shift 2
	#echo "RUNNING $@ in $1"
	$@  > $ENTRYPOINT_LOGS/$LOGFILENAME 2>&1
)
run_batch_packages_job() (
	...
	local MAX=$ENTRYPOINT_MAX_PARALLEL
	if [ $MAX = 1 ]; then
		for x in $ELT_LIST; do
			echo "$SCOPE:$x -> $CMD"
			cd $BASE/$x && $CMD > $ENTRYPOINT_LOGS/$SCOPE-$x.log 2>&1
		done
	else
		local pids=[]
		local COUNT=0
		...
		for x in $ELT_LIST; do
			...
			echo "   $x -> $CMD , logs $ENTRYPOINT_LOGS/$LOGFILENAME"
			#cd $BASE/$x && $CMD > $ENTRYPOINT_LOGS/$LOGFILENAME 2>&1 &
			run_one "$BASE/$x" "$ENTRYPOINT_LOGS/$LOGFILENAME" $CMD &
			pids[${i}]=$!
			COUNT=$(expr $COUNT + 1 )
